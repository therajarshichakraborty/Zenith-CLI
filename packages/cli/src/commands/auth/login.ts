...
export async function storeToken(token: any, serverUrl?: string) {
  try {
...
export async function requireAuth() {
  ...
  if (!token) {
    console.log(chalk.red("❌ Not authenticated. Please run 'zenith login' first."));
    process.exit(1);
  }

  if (await isTokenExpired()) {
    console.log(chalk.yellow("⚠️  Your session has expired. Please login again."));
    console.log(chalk.gray("   Run: zenith login\n"));
    process.exit(1);
  }
...
export async function loginAction(opts: any) {
  ...
  try {
    ...
    if (token) {
      const save = await storeToken(token, serverUrl);
...
export async function whoamiAction(opts: any) {
  ...
  try {
    ...
    console.log(chalk.red("❌ Session validation failed. Please login again."));
    process.exit(1);
  }
