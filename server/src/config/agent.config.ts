...
const ApplicationSchema = z.object({
  ...
  setupCommands: z
    .array(z.string())
    .describe("Bash commands to setup and run (e.g., npm/bun install, npm/bun run dev)"),
});

function printSystem(message: any) {
  console.log(message);
}
  ...
  Object.keys(filesByDir)
    ...
    .forEach((dir: any) => {
      if (dir) {
        ...
        filesByDir[dir].forEach((file: any) => {
...
async function createApplicationFiles(baseDir: any, folderName: any, files: any) {
...
export async function generateApplication(description: any, aiService: any, cwd = process.cwd()) {
  try {
    ...
    if (application.setupCommands && application.setupCommands.length > 0) {
      ...
      application.setupCommands.forEach((cmd: any) => {
    ...
    return {
      ...
      appDir,
      files: application.files.map((f: any) => f.path),
      commands: application.setupCommands || [],
      success: true,
    };
  } catch (err) {
    printSystem(chalk.red(`\n❌ Error generating application: ${(err as any).message}\n`));
    if ((err as any).stack) {
      printSystem(chalk.dim((err as any).stack + "\n"));
    }
    throw err;
  }
}
